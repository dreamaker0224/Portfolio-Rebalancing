...
def write_back(weights, stocks, market_value, data):
    """將投資組合更新傳回 app.py"""
    ...
    shares_data = {stock: int(total_shares[i] / latest_prices[stock]) for i, stock in enumerate(stocks) if total_shares[i] / latest_prices[stock] != 0}
...
def main(tau, require_return, stocks, date, last_holds,delta = 0.5):
    ...
    print("儲存結果...")
    return write_back(weights, stocks, market_value, data)
