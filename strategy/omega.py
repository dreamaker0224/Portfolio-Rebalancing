...
def optimize_portfolio(returns, delta, tau, required_return):
    ...
    model.setObjective(psi - cost, GRB.MAXIMIZE)
    ...
    model.addConstr(gp.quicksum(w[j] for j in range(n))+cost == 1, "Constraint_5")
    model.addConstr(gp.quicksum(r_j[j] * w[j] for j in range(n)) >= required_return, "Constraint_6")
    model.addConstr(gp.quicksum(r_j[j] * w[j] for j in range(n)) >= required_return, "Constraint_7")
    for i in range(n):
        model.addConstr(w[i] == w_buy[i]-w_sell[i], f"Constraint_8_{i}")
        model.addConstr(w[i] >= w_buy[i]*low, f"Constraint_9_{i}")
        model.addConstr(w[i] <= w_buy[i]*up, f"Constraint_10_{i}")
